...
class Ant(Insect):
    ...
    def double(self):
        ...
        "*** YOUR CODE HERE ***"
        my_place = self.place
        while my_place.exit != None:
            next_place = my_place.exit
            if next_place.ant != None:
                next_ant=next_place.ant
                if not next_ant.has_doubled:
                    next_ant.damage*=2
                    next_ant.has_doubled = True
                if next_ant.is_container and next_ant.ant_contained!=None:
                    if not next_ant.ant_contained.has_doubled:
                        next_ant.ant_contained.damage*=2
                        next_ant.ant_contained.has_doubled = True
            my_place = next_place
        # END Problem 12
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self):
        ...
        my_place = self.place
        index = 0
        for index in range(1,self.lower_bound+1):
            my_place = my_place.entrance
            if my_place == None:
                return None
        while not my_place.is_hive and index<=self.upper_bound:
            if my_place.bees != []:
                return random_bee(my_place.bees)
            my_place = my_place.entrance
            index+=1
        return None
...
class HungryAnt(Ant):
    ...
    def action(self, gamestate):
        if self.cooldown == 0:
            my_place = self.place
            bee_here = my_place.bees
            if bee_here != []:
                eat = random_bee(bee_here)
                eat.reduce_health(eat.health)
                bee_here = my_place.bees
